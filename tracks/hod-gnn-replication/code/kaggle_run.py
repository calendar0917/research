...
def run_graphvit(dataset, seed, smoke, max_epochs):
    ...
    # GraphViT uses YACS (not Hydra): update_cfg parses positional key/value
    # pairs after an optional --config argument.  Passing Hydra-style
    # ``cfg.seed=...`` strings makes YACS reject the override list as odd.
    config = f"train/configs/GraphMLPMixer/{dataset.replace('-', '_')}.yaml"
    cmd = [str(VENV_PY), "-m", mod, "--config", config,
           "seed", str(seed), "train.runs", "1"]
    # Keep the integer ``device`` field intact for YACS type checking.  The
    # vendored GraphViT runner maps it to an explicit CPU torch.device when
    # CUDA is unavailable.  Keep one worker/thread for CPU smoke checks.
    if RUNTIME_INFO.get("gpu_name") == "CPU-only":
        cmd += ["num_workers", "1"]
    if smoke:
        cmd += ["train.epochs", "2"]
    if max_epochs:
        cmd += ["train.epochs", str(max_epochs)]
    log = LOGS / f"graphvit-{dataset}-{seed}.log"
    started_wall = time.time()
    env = dict(os.environ, HOD_GNN_NO_STDOUT_REDIRECT="1", PYTHONUNBUFFERED="1")
    rc, secs, meta = run(cmd, repo, log, label=f"graphvit/{dataset}/seed={seed}", env=env)
    # Official GraphViT redirects stdout internally.  If that checkout does
    # not include our opt-out hook, recover the newest per-run internal log so
    # epoch metrics remain auditable and parseable on Kaggle as well.
    if log.exists() and log.stat().st_size == 0:
        candidates = []
        internal_root = repo / "logs"
        if internal_root.exists():
            for p in internal_root.rglob("*.txt"):
                try:
                    if p.stat().st_mtime >= started_wall - 2:
                        candidates.append(p)
                except OSError:
                    pass
        if candidates:
...
def parse_graphvit(log, metric):
    """Parse GraphViT's ``Epoch: ..., Val: ..., Test: ...`` rows."""
    number = r"([-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?)"
    rows = []
    pat = re.compile(rf"Epoch:\s*(\d+).*?Val:\s*{number}.*?Test:\s*{number}", re.I)
    with open(log, encoding="utf-8", errors="replace") as f:
        for line in f:
            m = pat.search(line)
            if m:
    ...
    if not rows:
        return {}
    best = min(rows, key=lambda x: x[1]) if metric == "mae" else max(rows, key=lambda x: x[1])
    return {"best_epoch": best[0], "best_val": best[1],
            "test_mae" if metric == "mae" else "test_auc": best[2]}
        ...
        for p in plan:
            ...
            if status == "success" and log:
                ...
                if m == "gps":
                    ...
                    g = parse_graphvit(log, metric) if m == "graphvit" else parse_generic(log)
