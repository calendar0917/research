#!/usr/bin/env python3
"""WL-Subtree-Kernel v1-semantics 变体（degree 节点标签 + 旧网格）。

仅用于 v1/v2 分解实验（label 效应 vs grid 效应），不是新的主实验。
v1 单文件脚本未被 git 追踪且已被 v2 覆盖；本文件由 v2 框架恢复 v1 语义
（v1 说明见 docs/README.md、notes/README.md；真正的上游源实现见
~/life/archive/科研/keyan.zip::phase2_structure_classification），供复现/对照：
  - 节点标签：REDDIT* = constant 0；其余（含 bio）= degree
  - 默认网格：n_iter {1,2,3,5}，C {0.1,1,10,100}
  - nested 默认 inner holdout 0.2（v1 语义；v2 已改为 cv5 + 1/9）
v1 官方结果保存在 results/ 顶层目录（未动），不受本文件影响。
"""
...
# GIN 对 WL 明确使用 h ∈ {1..6}
DEFAULT_GRID_ITERS = [1, 2, 3, 5]  # v1 语义
# v1 语义
DEFAULT_GRID_C = [0.1, 1.0, 10.0, 100.0]
...
def get_node_label_mode(dataset_name: str) -> str:
    """v1 语义：constant（REDDIT*）/ degree（其余，包括 bio 数据集）。"""
...
def main() -> None:
    ...
    ap.add_argument("--inner-strategy", choices=["holdout", "cv5"], default="holdout",
    ...
    ap.add_argument("--inner-val-frac", type=float, default=0.2,
                    help="v1 语义：holdout 内层验证占 outer-train 的 0.2（→ overall 72/18/10）")
