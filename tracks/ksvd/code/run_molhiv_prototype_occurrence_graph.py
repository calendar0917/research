"""Strict scaffold-fold pilot for prototype occurrence graphs.
...
* either no message passing (occurrence MIL) or one edge-aware GINE layer
  models interactions between occurrences;
* the supervised model never performs message passing on the original atom
  graph, so it cannot simply relearn and bypass the radius-2 prototype branch.

The random-real-prototype and KSVD families are matched under the same cosine
Top-K occurrence construction.  KSVD shuffled-ID and no-ID controls test
whether persistent dictionary identity matters beyond generic extra topology.
Only official-train scaffold folds are accepted; official-valid/test are never
evaluated or encoded by this runner.
"""
...
def main() -> None:
    ...
    ap.add_argument(
        "--controls",
        default=(
            "random_node_mil,random_occ_mil,random_occ_gine,"
            "ksvd_occ_mil,ksvd_occ_gine,"
            "ksvd_occ_gine_shuffled_id,ksvd_occ_gine_no_id"
        ),
    )
    ...
    required_families = sorted({c.family for c in controls if c.level == "occurrence"})
    ...
    report = {
        "protocol_id": "molhiv-prototype-occurrence-graph-scaffold-v1",
        "date": "2026-07-28",
        "hypothesis": (
            "explicit connected prototype occurrences plus at most one edge-aware "
            "occurrence GINE separates motif identity from motif interaction better "
            "than a strong original-node GINE backbone with an optional dictionary side channel"
        ),
        "architecture": {
            ...
            "occurrence_edges": "shared original atom and/or original molecular bond adjacency",
            "supervised_original_atom_graph_message_passing_layers": 0,
            "supervised_occurrence_message_passing_layers": "0 or 1 matched control",
            "graph_readout": "attention plus mean plus max",
        ...
        "selection_policy": {
            ...
            "seed0_promotion_rule_across_3_folds": {
                "occurrence_gine_mean_gain_over_occurrence_mil_at_least": 0.005,
                "minimum_occurrence_gine_fold_wins": 2,
                "ksvd_identity_must_beat_shuffled_mean": True,
                "ksvd_occurrence_gine_must_beat_random_occurrence_gine_mean": True,
            },
        },
        "config": vars(args),
        "fold": int(args.fold),
