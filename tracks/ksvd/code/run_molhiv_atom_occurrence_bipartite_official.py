...
def main() -> None:
    ...
    fit_indices = official_train.copy()
    heldout_indices = official_valid.copy()
    test_indices = official_test.copy()
    for left, right, name in (
        (fit_indices, heldout_indices, "train/valid"),
        (fit_indices, test_indices, "train/test"),
        (heldout_indices, test_indices, "valid/test"),
    ):
        if np.intersect1d(left, right).size:
            raise AssertionError(f"official {name} overlap")
    ...
    all_rows = np.concatenate([fit_rows, valid_rows, test_rows])
    if np.mean(np.linalg.norm(latents_np[all_rows], axis=1) > 0.99) < 0.999:
        raise ValueError("official train/valid/test latents are missing or unnormalized")

    if any(control.family != "farthest" for control in controls):
        raise ValueError("official runner is frozen to farthest real-patch controls only")

    # Exactly one deterministic observed patch per official-train graph prevents
    # large molecules from dominating the candidate pool.  Farthest selection
    # is label-free and seed-independent; seeds 0/1/2 vary only optimization.
    ...
    prototype_sets = {"farthest": farthest_prototypes}
    ...
    train_all = np.concatenate([fit_indices, heldout_indices, test_indices])
    ...
    def make_occurrence_collate(family: str, shuffled_identity: bool):
        if shuffled_identity:
            raise ValueError("official runner does not support shuffled identity")
    ...
    for control in controls:
        ...
        test_metrics = evaluate(
            model, control, make_loader(test_indices, False, 4000, collate_fn)
        )
        results[control.name] = {
            ...
            "official_valid": heldout_metrics,
        ...
        print(
            ...
            f"official_valid_auc={heldout_metrics['auc']:.4f} "
            f"official_test_auc={test_metrics['auc']:.4f}",
            flush=True,
        )
    ...
    report = {
        "protocol_id": "molhiv-realpatch-atom-occurrence-bipartite-full-official-v1",
        "date": "2026-07-28",
        "hypothesis": (
            "a frozen farthest real-patch atom-occurrence bipartite model can "
            "retain prototype incidence without a full atom-atom GNN"
        ),
        "architecture": {
            "local_metric": args.local_metric_description,
            "local_metric_gnn_layers": int(args.local_metric_gnn_layers),
            "prototype_vocabulary": "32 deterministic farthest observed official-train patches",
        ...
        "selection_policy": {
            ...
            "development_node_mil_mean_auc": 0.71731594,
            "development_bipartite_mean_auc": 0.73328369,
            "development_mean_gain": 0.01596775,
            "development_pairwise_wins": "6/9",
            "architecture_and_hyperparameters_frozen_before_this_run": True,
            "train_data": "official train only",
            "epoch_policy": f"fixed {args.epochs} epochs",
            "official_valid_evaluations_per_control": 1,
            "official_test_evaluations_per_control": 1,
            "post_test_tuning_permitted": False,
            "ensemble_policy": "predeclared arithmetic mean of model seeds 0/1/2 probabilities",
            "official_test_status": "controlled frozen evaluation, not untouched because repository previously inspected official test",
        },
        "config": vars(args),
        "n_train": int(len(fit_indices)),
        "n_train_positive": int(labels[fit_indices].sum()),
        "n_valid": int(len(heldout_indices)),
        "n_valid_positive": int(labels[heldout_indices].sum()),
        "n_test": int(len(test_indices)),
        "n_test_positive": int(labels[test_indices].sum()),
        "train_indices_sha256": _sha256(fit_indices),
        "valid_indices_sha256": _sha256(heldout_indices),
        "test_indices_sha256": _sha256(test_indices),
        ...
        "results": results,
        "comparisons": {
            "official_valid_bipartite_minus_node_mil_auc": (
                results.get("farthest_bipartite", {}).get("official_valid", {}).get("auc", 0.0)
                - results.get("farthest_node_mil", {}).get("official_valid", {}).get("auc", 0.0)
                if "farthest_bipartite" in results and "farthest_node_mil" in results else None
            ),
            "official_test_bipartite_minus_node_mil_auc": (
                results.get("farthest_bipartite", {}).get("official_test", {}).get("auc", 0.0)
                - results.get("farthest_node_mil", {}).get("official_test", {}).get("auc", 0.0)
                if "farthest_bipartite" in results and "farthest_node_mil" in results else None
            ),
        },
    ...
    print(json.dumps({
        "output": str(output),
        "seed": args.seed,
        "official_valid_auc": {name: row["official_valid"]["auc"] for name, row in results.items()},
        "official_test_auc": {name: row["official_test"]["auc"] for name, row in results.items()},
