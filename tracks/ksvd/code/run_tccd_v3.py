#!/usr/bin/env python
"""TCCD-v3 Strong-Local Bridge Audit runner.

Formal stages are GPU1-only by execution wrapper. Official ZINC test is never
loaded. The strong local encoder is frozen and embeddings are cached before
bridge training.
"""
...
def _load_records_for_training(args: argparse.Namespace):
    train_records, valid_records, meta = V.build_bridge_records(args.data_root, args.device)
...
def _train_arm(args: argparse.Namespace, arm: str) -> int:
    ...
    out: dict[str, Any] = {
        ...
        "seed": int(args.seed),
        "gpu": _device_provenance(args.device, args.physical_gpu),
        "input_representation": "frozen B-full e_struct in R^16",
        "adapter": "Linear(16,64)",
