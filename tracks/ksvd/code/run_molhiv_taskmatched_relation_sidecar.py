"""Task-matched relation-bank sidecar on frozen broad occurrence predictions.

Broad label-free farthest/scaffold banks provide the frozen graph-level base.
Nested task-aware or shuffled-label prototypes provide only compact exact-
distance 1+2 relation features. This separates vocabulary roles and tests
whether supervised matching is useful for relation composition even when its
standalone occurrence predictor is weaker. Official valid/test remain unused.
"""
...
def load_broad_pair_raw(
    ...
    required = ("graph_indices", "labels", "scores", "base_scores")
    if any(any(key not in raw for key in required) for raw in rows):
        raise ValueError("broad compact result must contain saved predictions")
    ...
    field = "base_scores" if variant == "broad_occurrence_pair" else "scores"
    probabilities = np.mean([
        _sigmoid(np.asarray(raw[field], dtype=np.float32)) for raw in rows
    ], axis=0)
...
def main() -> None:
    ...
    ap.add_argument("--base-result", required=True, help="task-matched result used for selector audit")
    ap.add_argument("--broad-result", required=True, help="saved broad compact-distance result")
    ...
    for name, doc in (("vocabulary", vocabulary), ("task base", prior), ("broad base", broad)):
        if int(doc.get("fold", -1)) != args.fold or doc.get("fit_indices_sha256") != fit_sha:
            raise ValueError(f"{name} fold/split mismatch")
    for name, doc in (("task base", prior), ("broad base", broad)):
        if int(doc.get("config", {}).get("seed", -1)) != args.seed:
    ...
    families = ("nested_taskaware", "nested_shuffled")
    ...
    base_variants = ("broad_occurrence_pair", "broad_relation_pair")
    base: dict[str, dict[str, dict[str, np.ndarray]]] = {
        variant: {split: load_broad_pair_raw(broad, split, variant) for split in ("fit", "heldout")}
        for variant in base_variants
    }
    for variant in base_variants:
        if not np.array_equal(base[variant]["fit"]["graph_indices"], fit_indices):
            raise AssertionError(f"{variant} fit prediction order mismatch")
        if not np.array_equal(base[variant]["heldout"]["graph_indices"], heldout_indices):
            raise AssertionError(f"{variant} heldout prediction order mismatch")
        if not np.array_equal(base[variant]["fit"]["labels"], labels[fit_indices]):
            raise AssertionError(f"{variant} fit labels mismatch")
        if not np.array_equal(base[variant]["heldout"]["labels"], labels[heldout_indices]):
            raise AssertionError(f"{variant} heldout labels mismatch")
    ...
    def train_head(
        ...
        print(
            f"{base_variant}_{family}_{ablation}_{mode} dim={fit_z.shape[1]} "
        ...
        for family in families:
            for ablation, bin_ids in ABLATIONS.items():
                for mode, store in (("real", compact_real), ("shuffled", compact_shuffled)):
                    ...
                    key = f"{base_variant}__{family}__{ablation}__{mode}"
                    results[key] = train_head(family, base_variant, ablation, mode, selected)
    ...
    doc = {
        "protocol_id": "molhiv_taskmatched_relation_sidecar_v1",
        "date": "2026-07-28",
        "hypothesis": (
            "A nested task-matched bank may add assignment-specific local relation signal when used "
            "only as a sidecar to strong frozen broad label-free occurrence/relation predictions."
        ),
        "selection_policy": {
            "development_data": "official-train only; three outer scaffold folds",
            "official_valid_evaluations": 0,
            "official_test_evaluations": 0,
            "status": "role-separated vocabulary sidecar diagnostic; no official split promotion",
        },
        "config": vars(args),
        "fold": args.fold,
        ...
        "base_predictions": {
            variant: {
                "fit_auc": float(roc_auc_score(base[variant]["fit"]["labels"], base[variant]["fit"]["scores"])),
                "heldout_auc": float(roc_auc_score(base[variant]["heldout"]["labels"], base[variant]["heldout"]["scores"])),
                "fit_score_sha256": sha256(base[variant]["fit"]["scores"].astype(np.float64)),
                "heldout_score_sha256": sha256(base[variant]["heldout"]["scores"].astype(np.float64)),
            }
            for variant in base_variants
        },
        "prototype_banks": {
