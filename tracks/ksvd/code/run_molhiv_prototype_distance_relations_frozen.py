"""Frozen-base bounded-linear prototype relation pilot on MolHIV scaffold folds.
...
def main() -> None:
    ...
    ap.add_argument("--relation-gate-scale", type=float, default=1.0, help="legacy compatibility; unused")
    ap.add_argument("--relation-epochs", type=int, default=40)
    ap.add_argument("--relation-batch-size", type=int, default=256)
    ap.add_argument("--relation-lr", type=float, default=5e-3)
    ap.add_argument("--relation-weight-decay", type=float, default=0.1)
    ap.add_argument("--max-relation-residual", type=float, default=0.5)
    ...
    if args.max_relation_residual <= 0 or args.standardization_clip <= 0:
        raise ValueError("relation residual bound and standardization clip must be positive")
    ...
    validate_upstream_result(
        vocabulary_doc, name="vocabulary", evaluation_split=args.evaluation_split,
        fold=args.fold, fit_sha=expected_fit_sha,
    )
    ...
    class PrototypeMIL(nn.Module):
        """The frozen occurrence baseline, kept byte-for-byte equivalent in behavior."""
    ...
    class BoundedLinearResidual(nn.Module):
        """A 2,650-parameter residual with a hard graph-logit magnitude bound."""

        def __init__(self):
            super().__init__()
            self.linear = nn.Linear(relation_dim, 1, bias=False)
            nn.init.zeros_(self.linear.weight)

        def forward(self, features):
            return args.max_relation_residual * torch.tanh(self.linear(features).view(-1))
    ...
    def evaluate_base(model, loader):
        ...
        raw = {"graph_indices": graph_indices, "labels": y, "scores": scores}
        return metrics, raw

    def train_base(family: str):
        seed_everything(args.seed, torch)
        collate_fn = make_collate(family, "real")
        model = PrototypeMIL(prototype_sets[family]).to(device)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            for batch in train_loader:
                ...
                seen += n
            history.append({"epoch": float(epoch), "task": total / max(seen, 1)})
            if epoch == 1 or epoch % 5 == 0 or epoch == args.epochs:
                print(
                    f"{family}_base epoch={epoch:02d} task={history[-1]['task']:.5f}",
                    flush=True,
                )
        fit_metrics, fit_raw = evaluate_base(
            model, make_loader(fit_indices, False, 2000, collate_fn)
        )
        heldout_metrics, heldout_raw = evaluate_base(
            model, make_loader(heldout_indices, False, 3000, collate_fn)
        )
        return model, history, fit_metrics, fit_raw, heldout_metrics, heldout_raw, time.time() - start

    def standardized_relation(source: np.ndarray):
        ...
        heldout_z = np.clip(
            (np.asarray(source[heldout_indices], dtype=np.float32) - mean) / safe_std,
    ...
    def train_relation_head(
        family: str,
        mode: str,
        base_fit: dict[str, np.ndarray],
        base_heldout: dict[str, np.ndarray],
    ):
        source = relation_real[family] if mode == "real" else relation_shuffled[family]
        fit_z, heldout_z, normalization = standardized_relation(source)
        if not np.array_equal(base_fit["graph_indices"], fit_indices):
            raise AssertionError("base fit prediction order changed")
        if not np.array_equal(base_heldout["graph_indices"], heldout_indices):
            raise AssertionError("base heldout prediction order changed")
        seed_everything(args.seed + 41, torch)
        head = BoundedLinearResidual().to(device)
        optimizer = torch.optim.AdamW(
            head.parameters(), lr=args.relation_lr, weight_decay=args.relation_weight_decay
        )
        x = torch.from_numpy(fit_z)
        base = torch.from_numpy(base_fit["scores"])
        y = torch.from_numpy(base_fit["labels"])
        dataset = torch.utils.data.TensorDataset(x, base, y)
        ...
        for epoch in range(1, args.relation_epochs + 1):
            ...
            if epoch == 1 or epoch % 10 == 0 or epoch == args.relation_epochs:
                print(
                    f"{family}_{mode}_frozen_linear epoch={epoch:02d} "
                    f"task={history[-1]['task']:.5f} w={history[-1]['weight_l2']:.4f}",
                    flush=True,
                )

        @torch.no_grad()
        def combine(z_np: np.ndarray, base_raw: dict[str, np.ndarray]):
            head.eval()
            residual = head(torch.from_numpy(z_np).to(device)).cpu().numpy().astype(np.float32)
            ...
            metrics = {
                ...
                "graph_indices_sha256": sha256(base_raw["graph_indices"]),
                "score_sha256": sha256(scores.astype(np.float64)),
                "base_score_sha256": sha256(base_raw["scores"].astype(np.float64)),
        ...
        return {
            "fit": combine(fit_z, base_fit),
            "heldout": combine(heldout_z, base_heldout),
    ...
    requested = {control.name for control in controls}
    results: dict[str, Any] = {}
    base_cache: dict[str, Any] = {}
    for family in required_families:
        prefix_name = "farthest" if family == "farthest" else "scaffold"
        occurrence_name = f"{prefix_name}_occurrence"
        real_name = f"{prefix_name}_relation"
        shuffled_name = f"{prefix_name}_relation_shuffled"
        model, base_history, fit_metrics, fit_raw, heldout_metrics, heldout_raw, base_elapsed = train_base(family)
        base_params = int(sum(p.numel() for p in model.parameters() if p.requires_grad))
        base_cache[family] = {
            "fit_raw": fit_raw,
        ...
        if occurrence_name in requested:
            results[occurrence_name] = {
                "family": family,
                "relation_mode": "none",
                "integration": "frozen occurrence baseline",
                "fit": fit_metrics,
                "heldout": heldout_metrics,
                "prototype_rows": prototype_rows[family].tolist(),
                "prototype_sha256": sha256(prototype_sets[family]),
                "trainable_parameters": base_params,
                "history": base_history,
                "elapsed_sec": base_elapsed,
            }
            print(f"{occurrence_name} heldout_auc={heldout_metrics['auc']:.4f}", flush=True)
        for name, mode in ((real_name, "real"), (shuffled_name, "shuffled")):
            if name not in requested:
                continue
            relation_result = train_relation_head(family, mode, fit_raw, heldout_raw)
            results[name] = {
                "family": family,
                "relation_mode": mode,
                "integration": "frozen base plus bounded feature-standardized linear residual",
                "fit": relation_result["fit"],
                "heldout": relation_result["heldout"],
                "prototype_rows": prototype_rows[family].tolist(),
                "prototype_sha256": sha256(prototype_sets[family]),
                "base_trainable_parameters": base_params,
                "relation_trainable_parameters": relation_result["relation_trainable_parameters"],
                "total_parameters_during_relation_fit": relation_result["relation_trainable_parameters"],
                "base_history": base_history,
                "relation_history": relation_result["history"],
                "normalization": relation_result["normalization"],
                "relation_weight_l2": relation_result["relation_weight_l2"],
                "relation_weight_sha256": relation_result["relation_weight_sha256"],
                "elapsed_sec": base_elapsed + relation_result["elapsed_sec"],
            }
            heldout = relation_result["heldout"]
            print(
                f"{name} heldout_auc={heldout['auc']:.4f} base_auc={heldout['base_auc']:.4f} "
                f"residual={heldout['mean_abs_relation_residual']:.5f} max={heldout['max_abs_relation_residual']:.5f}",
                flush=True,
            )

    comparisons: dict[str, float] = {}
    for prefix_name in ("farthest", "scaffold"):
        baseline = f"{prefix_name}_occurrence"
        real = f"{prefix_name}_relation"
        shuffled = f"{prefix_name}_relation_shuffled"
        if real in results and baseline in results:
            comparisons[f"{prefix_name}_relation_minus_occurrence_auc"] = float(
                results[real]["heldout"]["auc"] - results[baseline]["heldout"]["auc"]
            )
        if real in results and shuffled in results:
            comparisons[f"{prefix_name}_relation_minus_shuffled_auc"] = float(
    ...
    report = {
        "protocol_id": "molhiv-prototype-frozen-bounded-linear-distance-relations-scaffold-v1",
        "date": "2026-07-28",
        "hypothesis": (
            "exact distance relations add incremental signal when the strong occurrence baseline "
            "is frozen and the residual is low-parameter and magnitude-bounded"
        ),
        "architecture": {
            ...
            "relation_features": "upper triangles plus bin pair/mass scalars; fit-only feature standardization",
            "integration": "fully frozen occurrence PrototypeMIL plus zero-initialized bounded linear residual",
            "residual_parameter_count": int(relation_dim),
            "residual_absolute_bound": float(args.max_relation_residual),
            "shuffled_control": "deterministic node permutation over fixed molecular distance matrices",
        ...
        "selection_policy": {
            ...
            "fixed_base_epochs": args.epochs,
            "fixed_relation_epochs": args.relation_epochs,
