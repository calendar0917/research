"""Terminal official-split evaluation of frozen raw-patch prototype MIL.

The PCA metric and KSVD dictionary must already be fit on official-train only.
This runner trains fixed 30-epoch GNN-free node MIL models on official train and
performs one post-training evaluation on official valid and official test.
No epoch/model selection is performed here.
"""
...
def main() -> None:
    ...
    ap.add_argument('--controls', default='random_node_mil,ksvd_node_mil')
    ...
    if not controls or set(controls) - {'random_node_mil','ksvd_node_mil'}:
    ...
    train_rows=rows_for(tr,offsets)
    rng=np.random.default_rng(args.seed+910_003)
    candidate_rows=rng.choice(train_rows,size=args.candidate_bank_size,replace=False).astype(np.int64)
    bank=normalize_rows(latents_np[candidate_rows])
    random_selection=rng.choice(args.candidate_bank_size,size=args.n_prototypes,replace=False).astype(np.int64)
    prototypes={
        'random_node_mil':bank[random_selection],
        'ksvd_node_mil':normalize_rows(np.asarray(tok['dictionary_ksvd'],dtype=np.float32).T),
    }
    ...
    results={}
    for ci,name in enumerate(controls):
    ...
    report={'protocol_id':'molhiv-gnnfree-rawpatch-pca64-official-terminal-v1','date':'2026-07-28','seed':args.seed,
            ...
            'official_train_sha256':fit_hash,'latent_cache_sha256':sha256(latents_np),'dictionary_sha256':sha256(np.asarray(tok['dictionary_ksvd'])),'candidate_rows':candidate_rows.tolist(),'random_selection':random_selection.tolist(),'random_source_node_rows':candidate_rows[random_selection].tolist(),'results':results,'elapsed_sec':time.time()-t0,'output':args.output}
