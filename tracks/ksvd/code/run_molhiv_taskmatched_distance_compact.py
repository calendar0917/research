"""Frozen compact distance-1+2 residual for nested task-matched vocabularies.

The task-aware and shuffled-label vocabularies are selected using only each
outer-fit split by the audited nested selector. Their saved occurrence logits
are frozen. A 140-dimensional exact-distance 1+2 relation residual is then fit
with a matched node-assignment-shuffled control. Official valid/test remain
unencoded and unevaluated.
"""
...
def load_base_raw(prior: dict[str, Any], family: str, split: str) -> dict[str, np.ndarray]:
    ...
    if any(key not in raw for key in required):
        raise ValueError("task-matched base result must contain saved occurrence predictions")
    return {
...
def main() -> None:
    ...
    doc = {
        "protocol_id": "molhiv_taskmatched_distance_compact_v1",
        "date": "2026-07-28",
        "hypothesis": (
            "Nested task-matched prototype selection should improve occurrence quality and preserve "
            "assignment-specific exact-distance 1+2 relation signal beyond shuffled controls."
        ),
        "selection_policy": {
            "development_data": "official-train only; three outer scaffold folds",
            "official_valid_evaluations": 0,
            "official_test_evaluations": 0,
            "status": "task-matched vocabulary development diagnostic; no official split promotion",
        },
        "config": vars(args),
        "fold": args.fold,
